"use client"

import { useRef } from "react"
import { motion, useInView } from "framer-motion"
import { ArrowRight } from "lucide-react"

const projects = [
  {
    image:
      "https://images.unsplash.com/photo-1558618666-fcd25c85f82e?w=800&q=80",
    title: "Dachsanierung Charlottenburg",
    type: "Steildach & Dämmung",
    span: "md:col-span-2",
  },
  {
    image:
      "https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&q=80",
    title: "Bürogebäude Mitte",
    type: "Flachdach & WDVS",
    span: "",
  },
  {
    image:
      "https://images.unsplash.com/photo-1513694203232-719a280e022f?w=800&q=80",
    title: "Wohnanlage Prenzlauer Berg",
    type: "Fassadensanierung",
    span: "",
  },
  {
    image:
      "https://images.unsplash.com/photo-1508450859948-4e04fabaa4ea?w=800&q=80",
    title: "Solaranlage Spandau",
    type: "Photovoltaik & Gründach",
    span: "md:col-span-2",
  },
]

export default function Referenzen() {
  const ref = useRef(null)
  const inView = useInView(ref, { once: true, margin: "-100px" })

  return (
    <section id="referenzen" className="py-24 md:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-6 md:px-12" ref={ref}>
        {/* Header */}
        <motion.div
          className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12 gap-6"
          initial={{ opacity: 0, y: 30 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6 }}
        >
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-[2px] bg-brand-orange" />
              <span className="font-heading text-xs font-bold uppercase tracking-[3px] text-brand-orange">
                Referenzen
              </span>
            </div>
            <h2 className="font-display text-3xl md:text-5xl font-black text-brand-dark leading-tight">
              Unsere <span className="text-brand-orange">Projekte</span>
            </h2>
          </div>
          <a
            href="#"
            className="inline-flex items-center gap-2 px-6 py-3 border-2 border-brand-dark text-brand-dark rounded-full font-heading text-sm font-bold no-underline hover:bg-brand-dark hover:text-white transition-all"
          >
            Alle Projekte ansehen
            <ArrowRight className="w-4 h-4" />
          </a>
        </motion.div>

        {/* Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {projects.map((project, i) => (
            <motion.div
              key={project.title}
              className={`group relative rounded-2xl overflow-hidden cursor-pointer ${project.span}`}
              initial={{ opacity: 0, y: 40 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: i * 0.12 }}
            >
              <img
                src={project.image}
                alt={project.title}
                className="w-full h-[300px] md:h-[360px] object-cover transition-transform duration-700 group-hover:scale-105"
              />

              {/* Hover overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex items-end p-6">
                <div>
                  <span className="font-heading text-xs font-bold uppercase tracking-widest text-brand-orange mb-2 block">
                    {project.type}
                  </span>
                  <h3 className="font-heading text-xl font-bold text-white">
                    {project.title}
                  </h3>
                </div>
              </div>

              {/* Always-visible bottom gradient */}
              <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/50 to-transparent group-hover:opacity-0 transition-opacity duration-400 flex items-end p-6">
                <h3 className="font-heading text-lg font-bold text-white">
                  {project.title}
                </h3>
              </div>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  )
}
